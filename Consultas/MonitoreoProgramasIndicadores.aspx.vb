...
Partial Class Consultas_MonitoreoProgramasIndicadores
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        SqlDataSourceProgramasIndicadoresMetas.SelectParameters(0).DefaultValue = idIndicador
        ...
        connection.Open()
        Dim sql As String = "SELECT [DescripcionIndicador] FROM [Indicadores] WHERE [IdIndicador]=" + indicador
       ...
       "WHERE pim.Peso <> 0 AND pim.IdIndicadorDeMeta = @IdIndicadorDeMeta " & _
    ...
    Protected Sub link1_Click(sender As Object, e As EventArgs)
        ...
        Dim name As String = ASPxGridViewProgramasIndicadoresMetas.GetRowValues(index, "IdPrograma").ToString

        Dim idPrograma As String = ASPxGridViewProgramasIndicadoresMetas.GetRowValues(index, "IdProgramasPorIndicadorDeMeta").ToString

        Dim porP As String = ASPxGridViewProgramasIndicadoresMetas.GetRowValues(index, "PorcentajeEjecucion").ToString


        Response.Redirect("MonitoreoIndicadoresRelevantesProgramas.aspx?NameP=" + uf.QueryStringEncode(politica) + "&CodP=" + uf.QueryStringEncode(idPolitica) + "&Porc=" + uf.QueryStringEncode(porcentaje) + "&CodC=" + uf.QueryStringEncode(idComponente) + "&NameC=" + uf.QueryStringEncode(componente) + "&PorcC=" + uf.QueryStringEncode(porcentajeC) + "&CodM=" + uf.QueryStringEncode(idMeta) + "&NameM=" + uf.QueryStringEncode(Meta) + "&PorM=" + uf.QueryStringEncode(porcentajeM) + "&CodI=" + uf.QueryStringEncode(idIndicador) + "&NameI=" + uf.QueryStringEncode(indicador) + "&PorI=" + uf.QueryStringEncode(porcentajeI) + "&CodProg=" + uf.QueryStringEncode(idPrograma) + "&Prog=" + uf.QueryStringEncode(name) + "&PorcProg=" + uf.QueryStringEncode(porP))
    End Sub
End Class
